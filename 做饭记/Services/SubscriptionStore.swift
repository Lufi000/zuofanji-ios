...
enum SubscriptionProductIDs {
    static let monthlyAutoRenewable = "com.lufi000.zuofanji.monthly"
    static let oneMonthPass = "com.lufi000.zuofanji.one_month"
...
enum AISubscriptionAccessError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .subscriptionRequired:
            return "开通 Snap Recipe Plus 后即可使用 AI 识别。"
        case .monthlyQuotaExceeded:
            return "本月 AI 识别额度已用完。额度会在下个订阅周期自动刷新。"
        case .dailyQuotaExceeded:
            return "今天的 AI 识别额度已用完。每天最多可使用 \(AISubscriptionQuota.daily) 次，请明天再试。"
        }
    }
}
...
final class SubscriptionStore: ObservableObject {
    ...
    private func performProductsAndEntitlementsRefresh() async {
        ...
        do {
            ...
            loadingState = .failed("载入 App Store 套餐信息超时，请稍后重试")
        } catch {
            loadingState = .failed("无法从 App Store 载入订阅信息，请检查网络后重试")
    ...
    func purchaseAutoRenewableMonthlySubscription() async {
        await purchase(
            ...
            missingProductMessage: "连续包月订阅信息暂时不可用，请稍后重试"
        )
    }
    ...
    func purchaseOneMonthPass() async {
        await purchase(
            ...
            missingProductMessage: "单月套餐信息暂时不可用，请稍后重试"
        )
    }
    ...
    private func purchase(productID: String, initialProduct: Product?, missingProductMessage: String) async {
        ...
        do {
            ...
            case .success(let verification):
                ...
                userMessage = "Snap Recipe Plus 已启用"
            case .userCancelled:
                break
            case .pending:
                userMessage = "购买正在等待确认"
            @unknown default:
                userMessage = "购买状态暂时无法确认"
            }
        } catch StoreError.failedVerification {
            userMessage = "购买验证失败，请稍后再试"
        } catch {
            userMessage = error.localizedDescription
        }
    }
    ...
    func restorePurchases() async {
        do {
            ...
            userMessage = hasActiveAIPlan ? "Snap Recipe Plus 已恢复" : "没有找到可恢复的套餐"
        } catch {
            userMessage = error.localizedDescription
        }
    }
