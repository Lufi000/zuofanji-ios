...
final class ImageCutoutService {
    ...
    /// Convert the sticker silhouette into a thin contour for the scan animation.
    func generateTracingContour(from silhouette: UIImage, width: CGFloat = 10) -> UIImage? {
        guard let cgImage = silhouette.cgImage else { return nil }

        let source = CIImage(cgImage: cgImage)
        let erodeFilter = CIFilter.morphologyMinimum()
        erodeFilter.inputImage = source
        erodeFilter.radius = Float(width)

        guard let eroded = erodeFilter.outputImage else { return nil }

        let contourFilter = CIFilter.sourceOutCompositing()
        contourFilter.inputImage = source
        contourFilter.backgroundImage = eroded

        guard let contour = contourFilter.outputImage?.cropped(to: source.extent) else {
            return nil
        }

        return renderToUIImage(contour)
    }
