...
extension TagChipView {
    ...
    static func difficulty(_ value: Difficulty, isSelected: Bool = false, onTap: (() -> Void)? = nil) -> TagChipView {
        TagChipView(text: value.localizedName, color: AppTheme.tagDifficulty, isSelected: isSelected, onTap: onTap)
    }

    /// 菜式标签
    static func cuisine(_ value: Cuisine, isSelected: Bool = false, onTap: (() -> Void)? = nil) -> TagChipView {
        TagChipView(text: value.localizedName, color: AppTheme.tagCuisine, isSelected: isSelected, onTap: onTap)
    }

    /// 耗时标签
    static func cookingTime(_ value: CookingTime, isSelected: Bool = false, onTap: (() -> Void)? = nil) -> TagChipView {
        TagChipView(text: value.localizedName, color: AppTheme.tagCookingTime, isSelected: isSelected, onTap: onTap)
    }
}
